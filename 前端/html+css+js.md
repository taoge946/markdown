...
嵌套的函数时可以访问作用域内的变量的，而且可以访问外部函数中定义的counter变量，**当外部函数返回之后，其他任何代码都无法访问counter变量，只有内部的函数才能访问到它**。（因为conter是一个局部变量，所以外部无法改变这个值）
...
```javascript
//将对象o中命名为m()的方法替换另一个方法
//可以在调用原始函数的方法之前和之后记录日志消息
function trace(o,m){
    var orignal=o[m]//在闭包中保存原始方法
    o[m]=function(){
        console.log(new.Data(),'Entering:',m)
...
> `apply`接收参数的是数组，`call`接受参数列表，``bind`方法通过传入一个对象，返回一个` this `绑定了传入对象的新函数。这个函数的 `this`指向除了使用`new `时会被改变，其他情况下都不会改变。若为空默认是指向全局对象window。
...
- 重要的`var that=this`:
  ...
  ![img](https://img-blog.csdn.net/20180827184120395?watermark/2/text/aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3UwMTA4NjUxMzY=/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70)
  
- JS中数据类型检测方案：

  - typeof 优点：能够快速区分基本数据类型    `typeof(true)  -->boolean`

    ​			 缺点：不能将Object、Array和Null区分，都返回object

  - instanceof  优点：能够区分Array、Object和Function，适合用于判断自定义的类实例对象

    ​					缺点：Number，Boolean，String基本数据类型不能判断

    ![image-20220403222723340](C:\Users\tao'ge\AppData\Roaming\Typora\typora-user-images\image-20220403222723340.png)

  - Object.prototype.toString.call():     优点：精准判断数据类型

    ​															 缺点：写法繁琐不容易记，推荐进行封装后使用

    ![image-20220403222944126](C:\Users\tao'ge\AppData\Roaming\Typora\typora-user-images\image-20220403222944126.png)
...
## 标签自动补全： 
  ...
  ![img](https://img-blog.csdnimg.cn/20181107095113473.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzM1ODEyMzgw,size_16,color_FFFFFF,t_70)  
